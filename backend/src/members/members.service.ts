...
export class MembersService {
  ...
  async create(dto: CreateMemberDto, role: MemberRole = MemberRole.MEMBER): Promise<Member> {
    if (this.usesFirebase()) {
      const existing = await this.firestoreService.collection('profiles').where('email', '==', dto.email).limit(1).get();
      if (!existing.empty) throw new ConflictException('A member with this email already exists');
      const id = randomUUID(), now = new Date();
      const profile = { email: dto.email, name: dto.name, role, phone: (dto as any).phone ?? '', address: (dto as any).address ?? '', createdAt: now, updatedAt: now };
      await this.firestoreService.collection('profiles').doc(id).set(profile);
      return this.mapFirestore(id, profile);
    }
    if (this.sqliteService.findUserByEmail(dto.email)) throw new ConflictException('A member with this email already exists');
    const user = this.sqliteService.createUser({ ...dto, role } as any);
  ...
  async findAll(query?: string): Promise<Member[]> {
    if (this.usesFirebase()) {
      ...
      return snapshot.docs.map(doc => this.mapFirestore(doc.id, doc.data())).filter(m => !q || m.name.toLowerCase().includes(q) || m.email.toLowerCase().includes(q));
    }
    if (!this.sqliteService.isReady()) return [];
    const db = this.sqliteService.getDatabase(), term = `%${query ?? ''}%`;
    const users = query ? db.prepare('SELECT * FROM users WHERE name LIKE ? OR email LIKE ? ORDER BY name').all(term, term) : db.prepare('SELECT * FROM users ORDER BY name').all();
  ...
  async findByEmail(email: string): Promise<Member | undefined> {
    if (this.usesFirebase()) {
      const snapshot = await this.firestoreService.collection('profiles').where('email', '==', email).limit(1).get();
  ...
  async update(id: string, dto: UpdateMemberDto): Promise<Member> {
    if (this.usesFirebase()) {
      ...
      if (!(await ref.get()).exists) throw new NotFoundException(`Member with ID "${id}" not found`);
      await ref.update({ ...dto, updatedAt: new Date() }); return this.findOne(id);
    }
    ...
    if (!db.prepare('SELECT id FROM users WHERE id = ?').get(id)) throw new NotFoundException(`Member with ID "${id}" not found`);
    const fields = Object.entries(dto).filter(([, value]) => value !== undefined);
    if (fields.length) db.prepare(`UPDATE users SET ${fields.map(([key]) => `${key} = ?`).join(', ')}, updatedAt = ? WHERE id = ?`).run(...fields.map(([, value]) => value), new Date().toISOString(), id);
    return this.findOne(id);
  }
  async remove(id: string): Promise<void> {
    if (this.usesFirebase()) { await this.firestoreService.collection('profiles').doc(id).delete(); return; }
  ...
  private mapSqlite(user: any): Member {
    return { id: user.id, email: user.email, name: user.name, phone: user.phone || '', dateOfBirth: user.dateOfBirth ? new Date(user.dateOfBirth) : new Date(), address: user.address || '', preferences: user.preferences, paymentMethod: user.paymentMethod, paymentDetails: user.paymentDetails, role: this.role(user.role), createdAt: new Date(user.createdAt), updatedAt: new Date(user.updatedAt) };
  }
  private mapFirestore(id: string, data: any): Member {
    return { id, email: data.email ?? '', name: data.name ?? data.email ?? 'User', phone: data.phone ?? '', dateOfBirth: data.dateOfBirth ? new Date(data.dateOfBirth) : new Date(), address: data.address ?? '', preferences: data.preferences, paymentMethod: data.paymentMethod, paymentDetails: data.paymentDetails, role: this.role(data.role), createdAt: data.createdAt instanceof Date ? data.createdAt : new Date(data.createdAt ?? Date.now()), updatedAt: data.updatedAt instanceof Date ? data.updatedAt : new Date(data.updatedAt ?? Date.now()) };
  }
  private role(value?: string): MemberRole { return value?.toLowerCase() === 'admin' ? MemberRole.ADMIN : value?.toLowerCase() === 'librarian' ? MemberRole.LIBRARIAN : MemberRole.MEMBER; }
}
