    ...
    <Layout>
      <div className="py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-6">Manage Book Requests</h1>
          {loading && <p>Loading...</p>}
          {error && <p className="text-red-500">{error}</p>}
          {!loading && !error && (
            <BookRequestsList requests={requests} isAdmin={true} onUpdate={fetchRequests} />
          )}
        </div>
      </div>
    </Layout>
...
export default withAuth(AdminBookRequests, ['Admin']);
